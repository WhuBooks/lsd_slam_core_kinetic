...
ROSOutput3DWrapper::ROSOutput3DWrapper(int width, int height) {
  ...
  liveframe_publisher =
      nh_.advertise<lsd_slam_core::keyframeMsg>(liveframe_channel, 1);

  keyframe_channel = nh_.resolveName("lsd_slam/keyframes");
  keyframe_publisher =
      nh_.advertise<lsd_slam_core::keyframeMsg>(keyframe_channel, 1);

  graph_channel = nh_.resolveName("lsd_slam/graph");
  graph_publisher =
      nh_.advertise<lsd_slam_core::keyframeGraphMsg>(graph_channel, 1);
...
void ROSOutput3DWrapper::publishKeyframe(Frame* f) {
  ...
  memcpy(fMsg.camToWorld.data(), f->getScaledCamToWorld().cast<float>().data(),
         sizeof(float) * 7);
...
void ROSOutput3DWrapper::publishTrackedFrame(Frame* kf) {
  ...
  memcpy(fMsg.camToWorld.data(), kf->getScaledCamToWorld().cast<float>().data(),
         sizeof(float) * 7);
...
void ROSOutput3DWrapper::publishDebugInfo(Eigen::Matrix<float, 20, 1> data) {
  ...
  for (int i = 0; i < 20; i++) msg.data.push_back((float)(data[i]));
